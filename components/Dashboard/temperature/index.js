  ...
  return (
    <div className="w-10/12 mt-4 h-28 ring-8 ring-primary/20 dark:ring-secondary/20 bg-primary dark:bg-secondary rounded-xl dark:text-primary text-secondary p-8 flex items-center relative">
      ...
      <div className="flex flex-row items-center">
        ...
        <p className="mx-4 text-xl opacity-70">Feels Like {feelLike}°C</p>
        {/* Min and Max Temp for the day */}
        <div className="flex flex-col space-y-2">
          <p
            className="flex justify-center items-center font-semibold text-xl"
            title="Minimum Temperature"
          >
            <TrendingUpIcon className="w-8 mr-2 stroke-[1.6]" /> {tmax}°C
          </p>
          <p
            className="flex justify-center items-center font-semibold text-xl"
            title="Maximum Temperature"
          >
            <TrendingDownIcon className="w-8 mr-2 stroke-[1.6]" /> {tmin}°C
          </p>
        </div>
        {/* Humidity level */}
        <p className="flex text-xl font-semibold" title="Humidity">
          {MOISTURE_SVG}
          {humidity}
        </p>
      </div>
